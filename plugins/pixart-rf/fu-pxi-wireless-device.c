...
fu_pxi_wireless_device_get_cmd_response (FuPxiWirelessDevice *device,
					 ...
					 GError **error)
{
	FuPxiReceiverDevice *parent = fu_pxi_wireless_device_get_parent (FU_DEVICE (device) ,error);
	guint16 retry = 0;
	guint8 status = 0x0;
...
fu_pxi_wireless_device_fw_object_create (FuDevice *device, FuChunk *chk, GError **error)
{
	...
	if (!fu_pxi_wireless_device_set_feature (FU_DEVICE (parent), receiver_cmd->data, receiver_cmd->len, error))
		return FALSE;
